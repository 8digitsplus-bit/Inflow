...
- **Removed per-user pricing + team feature entirely** (revert to pre-per-user, single-user workspaces): deleted `sync_stripe_seat_count`, all `users`/`quantity`/seat multiplication, `seat_count`, `/api/org/seats` + all invite/member routes (organizations.py now only `/org/me`), `TeamSection`, `AcceptInvite` page/route, and all "unlimited team members/seats" copy. Checkout is a pure flat subscription (quantity 1); `users` param ignored. Updated AI prompt contexts (contact.py, support.py). Deleted obsolete tests (test_org_team, test_stripe_seat_sync). Verified: 12 pricing tests pass, `/org/seats`→404, `/org/me` OK, checkout ignores users (→$400), Settings/Choose Plan render clean.
- **Flat per-workspace pricing**: Essential $99/mo ($830/yr), Pro $149/mo ($1,250/yr), Enterprise $400/mo ($3,360/yr); yearly ≈30% below monthly-annualized (flat every year, no first-year coupon). Applied across all pricing surfaces.
