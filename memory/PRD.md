...
- **Flat per-workspace pricing** (replaced per-seat): Essential $99/mo ($830/yr), Pro $149/mo ($1,250/yr), Enterprise $400/mo ($3,360/yr) — all unlimited team members; yearly ~30% below monthly-annualized. Removed the seat selector (Choose Plan), disabled Stripe seat-sync (now a `flat_pricing` no-op), and made Enterprise seats unlimited in `/api/org/seats`. Updated across backend `SUBSCRIPTION_PLANS`, PricingSection, ChoosePlan, Checkout, Settings, TierGate, FAQ, TeamSection, and AI prompt contexts (contact.py, support.py). Verified: backend pytest 16/16, live Stripe checkout ($400/mo, users ignored), org/seats unlimited, and UI screenshots (landing monthly+yearly, Choose Plan, Checkout, Settings billing).
