...
- **Landing footer redesign**: Replaced `Footer` in `CTAFooter.js` with a 21st.dev-style animated footer (framer-motion staggered blur-in via `AnimatedContainer`, respects `prefers-reduced-motion`). Rounded-top glassmorphism panel with brand column (logo/tagline/copyright) + Product (#features/#pricing/#integrations/#faq), Company (Contact/Support/Sign In), Legal (Privacy/Terms/Cookies/Consent Preferences → Termly modal), Social (Twitter/LinkedIn/GitHub) columns. Added `id="integrations"` to ConnectBusinessSection for the anchor. Verified via screenshot.
